...
export class FriendComponent {
    ...
    Relavte: any[] = []; // Danh sách người đã gửi lời mời kết bạn
    ...
    ngOnInit() {
        this['loadFriends']();
        // this.getfriendRelative();
        const savedSent = localStorage.getItem('sentRequests');
        if (savedSent) {
            this.sentRequests = JSON.parse(savedSent);
        }
    ...
    loadFriends() {
        this.friendService.getListFriends().subscribe(
            (data: any) => {
                ...
                this.sentRequests = data.map((item: any) => item.id); // chỉ lấy danh sách ID người nhận
                console.log("Sent friend requests (BẠN gửi):", this.sentRequests);
    ...
    // getfriendRelative(){
    //     this.friendService.getRelativeMy(this.userService.getCookies()).subscribe(
    //         (data: any) => {
    //             this.friends = data; // Lưu cả id và name
    //             this.sentRequests = data.map((item: any) => item.id); // chỉ lấy danh sách ID người nhận
    //             console.log("Sent friend requests (BẠN gửi):", this.sentRequests);
